/**
 * Dashboard token data structure
 * Used for web-based dashboard access
 */
 ...
 */
export interface DashboardTokenValidationResult {
  valid: boolean;
  error?: 'not_found' | 'expired';
  token?: DashboardTokenData;
}

/**
 * Abstract repository for dashboard token operations
 */
export abstract class DashboardTokenRepository {
  /**
   ...
   */
  abstract validateToken(token: string): Promise<DashboardTokenValidationResult>;

  /**
   * Get a token by its value
   * @param token - The token to retrieve
   */
  abstract getToken(token: string): Promise<DashboardTokenData | null>;
   ...
   */
  abstract refreshToken(token: string, ttlMinutes?: number): Promise<DashboardTokenData | null>;
}
