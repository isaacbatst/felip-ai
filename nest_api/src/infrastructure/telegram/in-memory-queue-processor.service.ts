import { Queue } from '../queue/interfaces/queue.interface';
 ...
 */
export class InMemoryQueueProcessor<T> {
   ...
   */
  private async startProcessingForAllQueues(): Promise<void> {
    if (this.shouldStop || await this.queue.isEmpty()) {
      return;
    }
   ...
   */
  private async tryStartPendingQueues(): Promise<void> {
    ...
    for (const queueKey of pendingKeys) {
      ...
      if (this.processingKeys.has(queueKey) || await this.queue.isEmpty(queueKey)) {
  ...
  private async processNext(key?: string): Promise<void> {
    ...
    if (this.shouldStop || this.processingKeys.has(queueKey) || await this.queue.isEmpty(queueKey)) {
      return;
    }
